...
# release.sh - Release claude-warp to GitHub
...
# Get current version
CURRENT_VERSION=$(grep '^VERSION=' bin/claude-warp | cut -d'"' -f2)
...
# Update version in claude-warp
echo -e "${BLUE}Updating bin/claude-warp...${NC}"
sed -i '' "s/^VERSION=\".*\"/VERSION=\"${NEW_VERSION}\"/" bin/claude-warp
...
if [ -f "CHANGELOG.md" ]; then
    ...
    if ! grep -q "\[${NEW_VERSION}\]:" CHANGELOG.md; then
        sed -i '' "/^\[Unreleased\]:.*$/a\\
[${NEW_VERSION}]: https://github.com/cirrus-tools/claude-warp/compare/v${CURRENT_VERSION}...v${NEW_VERSION}
" CHANGELOG.md
    fi
fi
...
git add bin/claude-warp CHANGELOG.md
...
echo "     ./scripts/publish.sh claude-warp ${NEW_VERSION}"
echo ""
